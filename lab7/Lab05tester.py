...
def testNewClause(c1, c2, l1, l2, s, c, environment, idx):
    ...
    value = eval(
        "new_clause((THEC1, THEC2,"+l1+","+l2+", THESUBST))", environment)
    environment['THEVALUE'] = value
    Ptest = "new clause for " + \
        pf(c1, environment) + " / " + pf(c2, environment) + " with "
    Ptest += pf(l1, environment) + " / " + \
        pf(l2, environment) + " " + ps(s, environment)
    isSentence = False if value is None else \
        eval("is_sentence("+("'"+value+"'" if isinstance(value, str)
                             else 'THEVALUE')+")", environment)
    if isSentence:
    ...
    Pcheck = pf(c, environment)
    check = eval(c, environment)
    argsval = eval("get_args(THEVALUE)", environment, {
                   'THEVALUE': value}) if isSentence else []
#     print(argsval)
